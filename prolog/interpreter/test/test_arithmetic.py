...
class TestArithmeticMethod(object):
    def test_add(self):
        f1 = Float(5.1)
        f2 = Float(10.1)
        assert f1.arith_plus_sign(f2).floatval == 15.2

        n0 = Number(1)
        n1 = Number(2)
        assert n0.arith_plus_sign(n1).num == 3

        n2 = Number(2)
        f3 = Float(3.2)
        assert n2.arith_plus_sign(f3).floatval == 5.2
        assert f3.arith_plus_sign(n2).floatval == 5.2

        b1 = BigInt(rbigint.fromdecimalstr('50000000000000000000000'))
        b2 = BigInt(rbigint.fromdecimalstr('10000000000000000000001'))
        assert b1.arith_plus_sign(b2).value.str() == '60000000000000000000001'

        n3 = Number(sys.maxint)
        assert n3.arith_plus_sign(n0).value.str() == str(sys.maxint + 1)

        b = BigInt(rbigint.fromdecimalstr('100000000000000000000000000000'))
        f = Float(1.5)
        assert b.arith_plus_sign(f).floatval == 100000000000000000000000000001.5
        assert f.arith_plus_sign(b).floatval == 100000000000000000000000000001.5

        assert b.arith_plus_sign(n0).value.tofloat() == 100000000000000000000000000001.0
        assert n0.arith_plus_sign(b).value.tofloat() == 100000000000000000000000000001.0


    def test_sub(self):
        n1 = Number(5)
        n2 = Number(10)
        assert n1.arith_hyphenminus(n2).num == -5
        assert n2.arith_hyphenminus(n1).num == 5

        f1 = Float(10.5)
        f2 = Float(30.6)
        assert f1.arith_hyphenminus(f2).floatval == -20.1
        assert f2.arith_hyphenminus(f1).floatval == 20.1
    ...
    def test_div(self):
        ...
        py.test.raises(ZeroDivisionError, 'BigInt(rbigint.fromdecimalstr(\'1\')).arith_solidussolidus(Float(0))')
        py.test.raises(ZeroDivisionError, 'Float(1).arith_solidussolidus(Number(0))')
        py.test.raises(ZeroDivisionError, 'Number(1).arith_solidussolidus(Number(0))')
        py.test.raises(ZeroDivisionError, 'Number(1).arith_solidussolidus(Float(0))')
    ...
    def test_invert(self):
        assert Number(2345).arith_reverse_solidus().num == -2346
        assert BigInt(rbigint.fromdecimalstr('37846578346543875674385')).arith_reverse_solidus().value.str() == '-37846578346543875674386'
